...
class TargetROITab(QWidget):
    """서브탭 B: 타겟 이미지 ORB 시각화 + 멀티 ROI 설정"""
    def __init__(self):
        super().__init__()
        ...
        cv2_layout.addWidget(self._lbl("🔴 빨간 점 = ORB 특징점  |  파란 점선 = 설정된 ROI", bold=False))
        ...
        rv.addWidget(self._lbl("ℹ️  이미지 위에서 마우스를\n드래그하여 ROI를 추가하세요.", bold=False))
        rv.addStretch()
...
class VideoThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            if os.path.exists(active_file):
                try:
                    ...
                    pass
            if not model_path: model_path = os.path.join(_ROOT, "models", "canon_fast_yolo", "weights", "best.pt")
            if not os.path.exists(model_path): model_path = os.path.join(_ROOT, "yolov8n.pt")
        ...
        from engine.matcher import RESIZE_W, RESIZE_H, ROI_MATCH_THRESHOLD
        ...
        while self.running:
            ...
            if self.preprocessor and self.targets:
                ...
                if active_bbox:
                    ...
                    cv2.putText(frame, "Canon Monitor (YOLO)", (x1, y1 - 10),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)

            # 오버레이가 그려진 분석 프레임 우선 표시; 없으면 원본 폴백
            disp     = self._last_display_frame if self._last_display_frame is not None else frame
