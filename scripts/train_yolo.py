...
BASE_MODEL   = os.path.join(_ROOT, "yolov8n.pt")
...
def run_yolo_training(epochs: int = 30, imgsz: int = 640, progress_cb=None, mode: str = 'resume') -> dict:
    ...
    if mode == 'scratch':
        model_path = BASE_MODEL          # 처음부터: 사전학습 기본 모델 사용
    else:
        model_path = BEST_MODEL if os.path.exists(BEST_MODEL) else BASE_MODEL  # 이어서
    ...
    try:
        ...
        def _on_epoch_end(trainer):
            ...
            mp50  = m.get("metrics/mAP50(B)",     0.0)
            mp595 = m.get("metrics/mAP50-95(B)",  0.0)
            ...
            print(f"[METRIC] epoch={cur} total={total} "
                  ...
                  f"box_loss={box_l:.4f}", flush=True)
